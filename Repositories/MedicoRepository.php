...
class MedicoRepository {
    ...
    // Método para verificar si un médico tiene Cita asignadas
    public function tieneCitas(int $id): bool {
        // Sentencia SQL para contar las Cita asignadas a un médico
        $sql = "SELECT COUNT(*) AS total FROM Cita WHERE medico_id = :id";
        // Ejecutar la consulta y obtener el resultado
        $result = $this->conexion->fetchOne($sql, [':id' => $id]);

        // Si tiene Cita, devolver true, de lo contrario false
        return $result && $result['total'] > 0;
    }

    // Método para borrar un médico
    public function borrar(int $id): bool {
        // Verificar si el médico tiene Cita asignadas
        if ($this->tieneCitas($id)) {
            return false;  // Si tiene Cita, no se puede borrar
        }
